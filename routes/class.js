...
const { peopleDetectModel } = require("../model/model");

module.exports = router;

//Post Method
router.post("/post", async (req, res) => {
  const data = new peopleDetectModel({
...
router.get("/getAll/:limit/:page", async (req, res) => {
  try {
    ...
    const data = await peopleDetectModel.find().skip(skip).limit(limit);
    const totalItems = await peopleDetectModel.countDocuments(); // Count total items for pagination
...
router.get("/getOne/:id", async (req, res) => {
  try {
    const data = await peopleDetectModel.findById(req.params.id);
    res.json(data);
  } catch (error) {
    res.status(500).json({ message: error.message });
  }
...
router.patch("/update/:id", async (req, res) => {
  try {
    ...
    const result = await peopleDetectModel.findByIdAndUpdate(
      id,
      updatedData,
      options
    );
...
router.delete("/delete/:id", async (req, res) => {
  try {
    const id = req.params.id;
    const data = await peopleDetectModel.findByIdAndDelete(id);
